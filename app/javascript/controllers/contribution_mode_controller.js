...
export default class extends Controller {
  ...
  updateFieldsVisibility() {
    ...
    if (mode === "" || !mode) {
      ...
      // Calculate and display the contribution amount
      this.updateCalculatedAmount()
    }
  }

  // Update the contribution amount label between required and plain
  ...
  // Check if target_date is present and toggle warning/content visibility
  checkTargetDate() {
  ...
  // Calculate and update suggested target date for Fixed mode
  updateSuggestedDate() {
    const mode = this.modeInputTarget.value
    ...
    if (!targetAmountField || !currentAmountField || !contributionField) return
    ...
    const targetAmount = cleanValue(targetAmountField.value)
    const currentAmount = cleanValue(currentAmountField.value)
    const contributionAmount = cleanValue(contributionField.value)
    ...
    if (contributionAmount <= 0) {
      ...
      const enterAmountText = this.suggestedDateDisplayTarget.dataset.enterTargetAmountText
      this.suggestedDateDisplayTarget.value = enterAmountText
      this.replaceButtonTarget.disabled = true
      this.replaceButtonTarget.dataset.suggestedDate = ""
      return
    ...
    if (remaining <= 0) {
      const goalReachedText = this.suggestedDateDisplayTarget.dataset.goalReachedText
      this.suggestedDateDisplayTarget.value = goalReachedText
      this.replaceButtonTarget.disabled = true
      this.replaceButtonTarget.dataset.suggestedDate = ""
      return
    }

    // Get the contribution frequency
    ...
    // Calculate periods needed based on frequency
  ...
  // Replace target_date with suggested date
  ...
  // Calculate and update contribution amount for Calculated mode
  updateCalculatedAmount() {
    ...
    if (!targetAmountField || !currentAmountField || !targetDateField) return
    ...
    if (daysRemaining <= 0) {
      ...
      this.calculatedAmountValueTarget.textContent = `$${remaining.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`
      return
    }
    ...
    const contributionAmount = (remaining / periodsRemaining).toFixed(2)
    this.calculatedAmountValueTarget.textContent = `$${parseFloat(contributionAmount).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`
  }
}
