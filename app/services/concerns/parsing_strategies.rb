...
module ParsingStrategies
  ...
  def parse_ai_first(text_chunks, text)
    if ai_api_available?
      ...
      ai_result&.success? ? ai_result : success(parse_with_deterministic_parser(text))
    else
      success(parse_with_deterministic_parser(text))
    end
  end
  ...
  def parse_parser_first(text_chunks, text)
    ...
    if parser_result&.dig("transactions")&.any?
      ...
      ai_result = parse_with_ai(text_chunks, text)
      ai_result&.success? ? ai_result : success(empty_result)
    else
      success(empty_result)
    end
  end
