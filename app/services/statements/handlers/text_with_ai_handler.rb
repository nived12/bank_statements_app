...
module Statements
  module Handlers
    class TextWithAiHandler < ApplicationService
      ...
      # Process using parser result + AI categorization
      def process_with_parser_result(parsed_data)
        ...
        # Use AI to enhance/categorize parser-extracted transactions
        ...
        if enhanced_result.success? && enhanced_result.payload["transactions"].present?
          enhanced_transactions = enhanced_result.payload["transactions"]
          Rails.logger.info("AI enhanced #{enhanced_transactions.length} transactions")

          # Merge enhanced transactions back into parsed_data
          parsed_data["transactions"] = enhanced_transactions
          ...
          # Keep original parser result with basic extraction_source
        end
